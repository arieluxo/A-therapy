...
export default function ClientDashboard() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-20"><Loader2 className="h-6 w-6 animate-spin text-[#2D4A3E]" /></div>
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-light tracking-wider text-[#2D4A3E]">
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              {activeTraining && (
                <Button variant="ghost" size="sm" className="text-xs text-[#2D4A3E]" onClick={() => navigateTo('client-training')}>
            ...
            {activeTraining ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <Badge className="text-[10px] bg-[#2D4A3E] hover:bg-[#2D4A3E]">Activo</Badge>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {activeTraining.days?.slice(0, 6).map((day, i) => (
                    <div key={i} className="border rounded-lg p-2.5">
                      <p className="text-[10px] font-medium uppercase tracking-wider text-[#2D4A3E]">{day.dayName || `Día ${day.dayNumber}`}</p>
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="text-xs text-[#2D4A3E]" onClick={() => navigateTo('client-questionnaire')}>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              {client.sessions && client.sessions.length > 0 && (
                <Button variant="ghost" size="sm" className="text-xs text-[#2D4A3E]" onClick={() => navigateTo('client-sessions')}>
              ...
              <div className="space-y-3">
                ...
                {lastSession.totalVolume && lastSession.totalVolume > 0 && (
                  <div className="flex items-center gap-2 pt-2 border-t">
                    <BarChart3 className="h-3.5 w-3.5 text-[#2D4A3E]" />
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              {weightData.length > 3 && (
                <Button variant="ghost" size="sm" className="text-xs text-[#2D4A3E]" onClick={() => navigateTo('client-progress')}>
            ...
            {weightData.length > 1 ? (
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={weightData}>
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey="peso" stroke="#2D4A3E" strokeWidth={2} dot={{ r: 3, fill: '#2D4A3E' }} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
